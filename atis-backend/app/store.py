...
def sample_departures(stop_id: str):
    ...
    return [{
        ...
        "departure_in_min": h, "realtime": True } for h in headways]
def _base_options():
    return [
...
def _score_option(o, optimize="fastest", prefers_fewer_transfers=True):
    ...
    if prefers_fewer_transfers: score -= (1 - o["transfers"]) * 1.5
    if optimize == "reliable": score -= (o["reliability"] - 0.8) * 20
    if optimize == "least_walking": score += o["walk_km"] * 4
    return score
def sample_itineraries(origin, destination, prefers_fewer_transfers=True, optimize="fastest",
    ...
    if modes: options = [o for o in options if any(m in modes for m in o["modes"])]
    options = [o for o in options if o["walk_km"] <= max_walk_km]
    if avoid_stairs: options = [o for o in options if not o["stairs"]]
    if not bike_ok: options = [o for o in options if "bike" not in o["modes"]]
    for o in options: o["score"] = _score_option(o, optimize=optimize, prefers_fewer_transfers=prefers_fewer_transfers)
    options.sort(key=lambda x: x["score"])
    return options[:3]
def sample_weather(point, raw=False):
    import random as _r
    cond = _r.choice(["Clear", "Light rain", "Windy", "Overcast"])
    alert = None
    if cond in ("Light rain", "Windy"): alert = f"Take a small buffer: {cond.lower()} may slow services."
    data = {"condition": cond, "tempC": 16 + _r.randint(-2, 3), "windKph": 8 + _r.randint(0,12)}
